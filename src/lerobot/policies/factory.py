...
from .peft import load_peft_policy
from .pretrained import PreTrainedPolicy
from .processor_utils import flatten_feature_names as _flatten_feature_names
...
def make_pre_post_processors(
    ...
    **kwargs: Unpack[ProcessorConfigKwargs],
    ...
    """
    if pretrained_path:
        if policy_cfg.type == "perceptron_isaac":
            from .perceptron_isaac.processor_perceptron_isaac import (
                make_perceptron_isaac_pre_post_processors_from_pretrained,
            )

            retained_processor_root = getattr(policy_cfg, "_pretrained_processor_root", None)
            processor_path = str(retained_processor_root or pretrained_path)
            return make_perceptron_isaac_pre_post_processors_from_pretrained(
                config=policy_cfg,
                pretrained_path=processor_path,
                revision=None if retained_processor_root else pretrained_revision,
...
def make_policy(
    ...
    # A dataset's ``names`` is either a flat list of per-dimension names or a dict grouping
    # them by motor group (e.g. ``{"motors": ["x", "y", ...]}``); ``list()`` on the latter
    # yields the group keys, not the names, so flatten it explicitly.
    if ds_meta is not None:
        _set_action_feature_names_from_dataset(cfg, ds_meta.features)
    if ds_meta is not None:
    ...
    if cfg.pretrained_path and not cfg.use_peft:
        ...
        policy = load_peft_policy(
            policy_cls,
            cfg,
            cfg.pretrained_path,
            adapter_revision=cfg.pretrained_revision,
            is_trainable=True,
...
def _set_action_feature_names_from_dataset(cfg: PreTrainedConfig, dataset_features: dict[str, Any]) -> None:
    if not hasattr(cfg, "action_feature_names"):
        return
    action_names = _flatten_feature_names(dataset_features.get(ACTION, {}).get("names"))
    # Authenticated hardware packages own an exact wire-order contract. Dataset names can
    # be semantic aliases (for example ``main_shoulder_pan`` in older SO100 data), and
    # replacing the hardware names here makes a fine-tuned checkpoint fail its own strict
    # reload validation. Rename maps handle dataset keys; they must not rewrite the
    # package-owned action layout.
    if action_names is not None and not getattr(cfg, "strict_hardware_feature_contract", False):
