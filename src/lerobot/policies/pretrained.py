...
from lerobot.utils.hub import HubMixin, hub_snapshot_revision
...
class PreTrainedPolicy(nn.Module, HubMixin, abc.ABC):
    ...
    def save_pretrained(
        ...
        **push_to_hub_kwargs,
        ...
        if finalize:
            self.finalize_pretrained_package(save_directory)
        if push_to_hub:
            if repo_id is None:
    ...
    def _save_pretrained(self, save_directory: Path, state_dict: dict[str, Tensor] | None = None) -> None:
        ...
        for weight_path in (
            ...
            save_directory / "model.safetensors.index.json",
            *save_directory.glob("model-*.safetensors"),
        ):
    ...
    def finalize_pretrained_package(self, save_directory: str | Path) -> None:
        """Finalize policy-specific package metadata after every package writer has run."""

    def hub_delete_patterns(self) -> str | list[str] | None:
        """Return remote artifact patterns that this policy owns and may replace."""
        return None

    def hub_ignore_patterns(self) -> list[str] | None:
        """Return staged files that this policy intentionally excludes from Hub uploads."""
        return ["*.tmp", "*.log"]
    ...
    def push_model_to_hub(
        ...
        with TemporaryDirectory(ignore_cleanup_errors=True) as tmp:
            ...
            if peft_model is not None:
                ...
                peft_model.save_pretrained(saved_path, state_dict=state_dict)
                self.config.save_pretrained(saved_path)
            else:
                # Calls _save_pretrained and stores model tensors
                self.save_pretrained(saved_path, state_dict=state_dict, finalize=False)
            ...
            self.finalize_pretrained_package(saved_path)
            ...
            commit_info = api.upload_folder(
                ...
                ignore_patterns=self.hub_ignore_patterns(),
                delete_patterns=self.hub_delete_patterns(),
            )
    ...
    def wrap_with_peft(
        ...
        if self.config.pretrained_path:
            base_reference = str(self.config.pretrained_path)
            self.name_or_path = base_reference
            final_config.base_model_name_or_path = base_reference
            final_config.revision = self.config.pretrained_revision
            local_reference = Path(base_reference)
            embed_base_on_save = local_reference.is_dir() or local_reference.is_absolute()
            if not embed_base_on_save:
                try:
                    validate_repo_id(base_reference)
                except HFValidationError:
                    embed_base_on_save = True
            base_revision = self.config.pretrained_revision
            checkpoint_source = getattr(self.config, "_checkpoint_source_root", None)
            if not embed_base_on_save and checkpoint_source:
                with suppress(OSError, ValueError):
                    base_revision = hub_snapshot_revision(checkpoint_source)
            self.config._embed_base_on_save = embed_base_on_save
            self.config._peft_base_reference = base_reference
            self.config._peft_base_revision = base_revision
            final_config.revision = base_revision
