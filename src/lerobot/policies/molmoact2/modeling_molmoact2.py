...
_REMOTE_CODE_IGNORE_PATTERNS = ["*.py", "*.pyc", "__pycache__/*"]
...
def _resolve_molmoact2_location(
    tokenizer_path: str, *, revision: str | None = None, force_download: bool = False
) -> str:
    return str(
        resolve_hub_snapshot(
            ...
            ignore_patterns=_REMOTE_CODE_IGNORE_PATTERNS,
            token=_hf_token(),
        )
    )
...
def _resolve_checkpoint_location(
    ...
    *,
    ...
    return _resolve_molmoact2_location(checkpoint_path, revision=revision, force_download=force_download)
...
def _read_checkpoint_action_mode(config_path: Path) -> str | None:
...
def _saved_policy_action_mode(config: MolmoAct2Config) -> str | None:
    """Read the training mode from the saved package or underlying HF checkpoint."""
    pretrained_path = getattr(config, "pretrained_path", None)
    if pretrained_path is not None:
        mode = _read_checkpoint_action_mode(Path(pretrained_path) / "config.json")
        if mode is not None:
            return mode
    ...
    return _read_checkpoint_action_mode(checkpoint_location / "config.json")


def _apply_checkpoint_action_mode_for_inference(config: MolmoAct2Config) -> str | None:
    """Use checkpoint metadata as the source of truth for inference capabilities."""
    checkpoint_action_mode = _saved_policy_action_mode(config)
    if config.inference_action_mode is not None and checkpoint_action_mode is not None:
        config.action_mode = checkpoint_action_mode
    return checkpoint_action_mode
...
class MolmoAct2Policy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._checkpoint_action_mode = _apply_checkpoint_action_mode_for_inference(self.config)
