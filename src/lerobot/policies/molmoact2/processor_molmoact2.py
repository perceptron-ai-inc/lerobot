...
from lerobot.policies.processor_utils import coerce_batch_text, coerce_rgb_uint8, flatten_feature_names
...
def _flatten_feature_names(raw_names: Any) -> list[str] | None:
    return flatten_feature_names(raw_names, empty_as_none=True)
...
class MolmoAct2PackInputsProcessorStep(ProcessorStep):
    ...
    def _extract_images(self, observation: dict[str, Any], batch_size: int) -> list[list[np.ndarray]]:
        ...
        for key in self._resolve_image_keys(observation):
            ...
            for batch_idx in range(batch_size):
                ...
                images_by_example[batch_idx].append(
                    coerce_rgb_uint8(
                        item,
                        squeeze_leading_singletons=True,
                        shape_error="Unsupported image shape for MolmoAct2: {shape}.",
                    )
                )
    ...
    def __call__(self, transition: EnvTransition) -> EnvTransition:
        ...
        tasks = coerce_batch_text(
            task_source,
            batch_size,
            normalizer=_normalize_question_text if self.normalize_language else None,
        )
