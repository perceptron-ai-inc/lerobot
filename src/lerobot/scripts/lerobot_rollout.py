...
def rollout(cfg: RolloutConfig):
    ...
    with exclusive_torch_device_lock(
        cfg.device,
        enabled=cfg.exclusive_device_lock,
    ):
        _run_rollout(cfg)


def _run_rollout(cfg: RolloutConfig):
    """Run one rollout while the caller holds any required device lock."""
