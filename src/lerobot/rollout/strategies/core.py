...
from ..inference import InferenceEngine, SyncInferenceConfig
...
class RolloutStrategy(abc.ABC):
    ...
    def _init_engine(self, ctx: RolloutContext) -> None:
        ...
        self._run_prediction_preflight(ctx)
        logger.info("Inference engine started")

    def _run_prediction_preflight(self, ctx: RolloutContext) -> None:
        """Warm synchronous inference and validate outputs without sending commands."""
        count = int(ctx.runtime.cfg.preflight_inferences)
        if count == 0:
            return
        if not isinstance(ctx.runtime.cfg.inference, SyncInferenceConfig):
            logger.info("Prediction preflight is handled by the asynchronous inference backend")
            return

        engine = self._engine
        ordered_keys = ctx.data.ordered_action_keys
        logger.info("Running %d prediction-only preflight inference(s)...", count)
        for _ in range(count):
            ...
            actions = torch.as_tensor(engine.preflight(obs_frame))
            if actions.ndim != 2 or actions.shape[1] != len(ordered_keys):
                raise ValueError(
                    "Inference preflight must produce a [T, A] chunk with "
                    f"A={len(ordered_keys)}, got shape={tuple(actions.shape)}."
                )
            if not torch.isfinite(actions).all():
                raise ValueError("Inference preflight produced a non-finite action chunk.")
            logger.info("Validated complete prediction chunk with shape=%s", tuple(actions.shape))

        # The real rollout must begin at episode frame zero with empty policy,
        # processor, and interpolation state.
        engine.reset()
        self._interpolator.reset()
        self._cached_obs_processed = None
        logger.info("Prediction-only preflight passed; inference state reset")
    ...
    def _handle_warmup(self, use_torch_compile: bool, loop_start: float, control_interval: float) -> bool:
        ...
        if engine.failed:
            raise RuntimeError("Asynchronous inference backend failed during warmup.")
        if not engine.ready:
    ...
    def _teardown_context(self, ctx: RolloutContext) -> None:
        """Tear down one rollout using its action-dispatch safety policy."""
        self._teardown_hardware(
            ctx.hardware,
            return_to_initial_position=(
                ctx.runtime.cfg.return_to_initial_position and ctx.runtime.cfg.execute_actions
            ),
        )
...
def send_next_action(
    ...
    if ctx.runtime.cfg.execute_actions:
