...
class BaseStrategy(RolloutStrategy):
    ...
    def run(self, ctx: RolloutContext) -> None:
        ...
        # Prediction-only warmup belongs to setup, not to the requested live
        # execution duration. Async backends may need several full inferences
        # before publishing their first chunk.
        ...
        while not ctx.runtime.shutdown_event.is_set():
            ...
            if (
                ...
                and cfg.duration > 0
                and (time.perf_counter() - start_time) >= cfg.duration
            ):
            ...
            if start_time is None:
                ...
                logger.info("Inference warmup complete; starting the %.1fs execution window", cfg.duration)
        ...
        if engine.failed:
            raise RuntimeError("Inference backend failed; see the producer error immediately above.")

    def teardown(self, ctx: RolloutContext) -> None:
        """Disconnect hardware and stop inference."""
        self._teardown_context(ctx)
