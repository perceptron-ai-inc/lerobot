...
class RolloutConfig:
    ...
    # Inference backend (polymorphic: --inference.type=sync|rtc|temporal_ensemble)
    ...
    # Runtime
    # None adopts checkpoint-owned timing when the policy publishes target_fps,
    # otherwise the generic LeRobot fallback is 30 Hz. An explicit mismatch is
    # rejected because it changes episode-relative model timestamps.
    fps: float | None = None
    ...
    # Run the complete observation -> policy -> postprocessor path without
    # dispatching commands. Intended for camera/state and torque-off preflight.
    execute_actions: bool = True
    # Number of synchronous inference calls to complete before a strategy can
    # enter its action-dispatch loop. These predictions are never sent.
    preflight_inferences: int = 1
    # Fail if another LeRobot rollout owns the selected CUDA device.
    exclusive_device_lock: bool = True
    ...
    def __post_init__(self):
        ...
        packaged_fps = getattr(self.policy, "target_fps", None)
        if self.fps is None:
            self.fps = float(packaged_fps) if packaged_fps is not None else 30.0
        elif packaged_fps is not None and abs(float(self.fps) - float(packaged_fps)) > 1e-9:
            # Deliberate: action chunks are timed for target_fps, so running a real robot off
            # that rate is a safety issue, not a preference. Pinned by
            # test_rollout_config_adopts_and_enforces_checkpoint_fps. To run slower, repackage
            # the checkpoint with the intended target_fps rather than overriding here.
            raise ValueError(f"Rollout fps={self.fps} does not match checkpoint target_fps={packaged_fps}.")
        self.fps = float(self.fps)
        if self.fps <= 0.0:
            raise ValueError("Rollout fps must be positive.")
        if self.dataset is not None and abs(float(self.dataset.fps) - self.fps) > 1e-9:
            # DatasetRecordConfig.fps defaults to 30 independently of the checkpoint, so a
            # 20 Hz package made every default recording command fail until the user hunted
            # down --dataset.fps. Adopt the resolved rate unless the user set it themselves.
            if "fps" in parser.get_explicit_override_fields("dataset"):
                raise ValueError(
                    f"Explicit dataset.fps={self.dataset.fps} does not match resolved rollout fps={self.fps}."
                )
            logger.info(
                "Adopting rollout fps=%s for dataset recording (was the %s default).",
                self.fps,
                self.dataset.fps,
            )
            self.dataset.fps = int(self.fps) if float(self.fps).is_integer() else self.fps
        if self.preflight_inferences < 0:
            raise ValueError("preflight_inferences must be nonnegative.")
        if not self.execute_actions and not isinstance(self.strategy, BaseStrategyConfig):
            raise ValueError("execute_actions=false is supported only by the non-recording base strategy.")
