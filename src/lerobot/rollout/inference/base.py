...
class InferenceEngine(abc.ABC):
    ...
    def preflight(self, obs_frame: dict) -> torch.Tensor:
        """Generate output for prediction-only validation.

        The default validates one ordinary action. Chunk-native engines may
        override this to validate a complete independently generated plan.
        """

        action = self.get_action(obs_frame)
        if action is None:
            raise RuntimeError("Inference preflight returned no action.")
        return torch.as_tensor(action).reshape(1, -1)
