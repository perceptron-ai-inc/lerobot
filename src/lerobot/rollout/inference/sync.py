...
class SyncInferenceEngine(InferenceEngine):
    ...
    def preflight(self, obs_frame: dict) -> torch.Tensor:
        """Generate, postprocess, reorder, and return one complete fresh chunk."""

        # Reset before every preflight pass so each pass invokes the model instead
        # of popping another element from a previously cached select_action queue.
        self.reset()
        observation = copy(obs_frame)
        autocast_ctx = (
            torch.autocast(device_type=self._device.type)
        ...
        with torch.inference_mode(), autocast_ctx:
            ...
            try:
                ...
                actions = self._policy.select_action(observation)
        ...
        if actions.ndim == 3 and actions.shape[0] == 1:
            ...
            raise ValueError(f"Inference preflight expected [T, A], got {tuple(actions.shape)}.")

        ordered = []
        for action in actions:
            action_dict = make_robot_action(action.detach().cpu(), self._dataset_features)
            ordered.append([action_dict[key] for key in self._ordered_action_keys])
