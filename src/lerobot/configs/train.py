...
class TrainPipelineConfig(HubMixin):
    ...
    num_workers: int = 4
    batch_size: int = 8
    # Number of microbatches per optimizer update. ``steps``, scheduler
    # progress, evaluation, and checkpoint frequencies remain optimizer-step
    # based; effective global batch is batch_size * this value * world size.
    ...
    # DDP find_unused_parameters. Defaults to True, which is required by policies with
    # conditional computation but costs an extra autograd-graph traversal every
    # iteration. Set to False when every trainable parameter is used in every forward
    # pass -- PyTorch logs "did not find any unused parameters in the forward pass"
    # when that is the case. A wrong False raises a loud DDP reduction error on the
    # first steps rather than corrupting gradients silently.
    ...
    def validate(self) -> None:
        ...
        if self.env is not None and self.policy is not None:
            self.env.reconcile_policy_contract(
                self.policy,
