...
def prepare_raw_observation(
    ...
    ``image_key_map`` is the physical-camera to policy-slot rename applied later by the
    policy preprocessor. Resizing happens before that preprocessor, so use the mapped key
    for the target shape while retaining the physical key in the returned observation.
    """
    ...
    for key in image_keys:
        policy_key = (image_key_map or {}).get(key, key)
        try:
            target_shape = policy_image_features[policy_key].shape
        except KeyError as exc:
            raise KeyError(
                f"robot image feature {key!r} maps to policy feature {policy_key!r}, "
                "but that feature is absent from the policy config"
            ) from exc
        resized_images[key] = resize_robot_observation_image(torch.tensor(lerobot_obs[key]), target_shape)
...
class RemotePolicyConfig:
    ...
    # Appended after the existing optional fields to preserve positional
    # construction compatibility with older clients.
