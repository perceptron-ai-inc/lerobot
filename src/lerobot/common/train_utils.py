...
def load_training_metadata(checkpoint_dir: Path) -> dict:
    """Return checkpoint resume metadata, including fields absent from older checkpoints."""
    return load_json(checkpoint_dir / TRAINING_STATE_DIR / TRAINING_STEP)
...
def save_checkpoint(
    ...
    Args:
        ...
        batch_size (int | None, optional): Per-process batch size to record for sample-exact
            resume. Defaults to None (not recorded).
        gradient_accumulation_steps (int | None, optional): Microbatches per optimizer update to
            record for sample-exact resume. Defaults to None (not recorded).
    ...
    policy.save_pretrained(pretrained_dir, state_dict=model_state_dict, finalize=False)
    cfg.save_pretrained(pretrained_dir)
    if getattr(policy.config, "use_peft", False) is True:
    ...
    policy.finalize_pretrained_package(pretrained_dir)
    save_training_state(
        checkpoint_dir,
        step,
        optimizer,
...
def load_fsdp_optimizer_state(model, optimizer, checkpoint_dir: Path) -> None:
    ...
    # The resharded state comes back on CPU (FULL_STATE_DICT materializes there), while the FSDP
    # flat params live on the local device. Adam's fused/foreach path groups params, grads and
    # exp_avg/exp_avg_sq by (device, dtype) and raises "Tensors of the same index must be on the
    # same device and the same dtype" on the first step() if they disagree. `step` is exempt --
    # torch explicitly allows it to stay CPU -- so leave it alone.
        ...
        for p in group["params"]:
            ...
            for key, value in state.items():
                if key == "step" or not torch.is_tensor(value):
                    continue
                if value.device != p.device or (value.is_floating_point() and value.dtype != p.dtype):
                    state[key] = value.to(
                        device=p.device, dtype=p.dtype if value.is_floating_point() else value.dtype
                    )
