...
def test_inference_action_mode_uses_hf_checkpoint_training_mode(tmp_path):
    (tmp_path / "config.json").write_text(
        json.dumps({"action_mode": "continuous"}),
        encoding="utf-8",
    )
    ...
    checkpoint_mode = molmoact2_modeling._apply_checkpoint_action_mode_for_inference(config)

    assert checkpoint_mode == "continuous"
    assert config.action_mode == "continuous"

    training_config = MolmoAct2Config(
        checkpoint_path=str(tmp_path),
        action_mode="both",
        inference_action_mode=None,
    )
    molmoact2_modeling._apply_checkpoint_action_mode_for_inference(training_config)

    assert training_config.action_mode == "both"
