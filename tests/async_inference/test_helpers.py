...
def test_raw_observation_uses_mapped_policy_shape_before_rename():
    """Physical camera keys resize against their mapped policy slots."""
    robot_obs = _create_mock_robot_observation()
    lerobot_features = _create_mock_lerobot_features()
    policy_image_features = {
        f"{OBS_IMAGES}.top": PolicyFeature(type=FeatureType.VISUAL, shape=(3, 96, 128)),
        f"{OBS_IMAGES}.side": PolicyFeature(type=FeatureType.VISUAL, shape=(3, 64, 80)),
    }
    image_key_map = {
        f"{OBS_IMAGES}.laptop": f"{OBS_IMAGES}.top",
        f"{OBS_IMAGES}.phone": f"{OBS_IMAGES}.side",
    }

    observation = raw_observation_to_observation(
        robot_obs, lerobot_features, policy_image_features, image_key_map=image_key_map
    )

    assert observation[f"{OBS_IMAGES}.laptop"].shape == (1, 3, 96, 128)
    assert observation[f"{OBS_IMAGES}.phone"].shape == (1, 3, 64, 80)
