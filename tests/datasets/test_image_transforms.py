...
def test_letterbox_resize_preserves_four_by_three_geometry():
    image = torch.ones(3, 480, 640)
    transform = LetterboxResize(size=(256, 256), fill=0.0)

    output = transform(image)

    assert output.shape == (3, 256, 256)
    assert not output[:, :32].any()
    assert not output[:, 224:].any()
    torch.testing.assert_close(output[:, 32:224], torch.ones(3, 192, 256))


def test_letterbox_resize_is_available_from_image_transform_config():
    cfg = ImageTransformsConfig(
        enable=True,
        max_num_transforms=1,
        tfs={
            "letterbox": ImageTransformConfig(
                type="LetterboxResize", kwargs={"size": (256, 256), "fill": 0.0}
            )
        },
    )
    transforms = ImageTransforms(cfg)

    output = transforms(torch.ones(3, 480, 640))

    assert isinstance(transforms.transforms["letterbox"], LetterboxResize)
    assert output.shape == (3, 256, 256)
