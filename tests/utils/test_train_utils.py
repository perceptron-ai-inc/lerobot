...
def test_save_training_state_records_num_processes(tmp_path, optimizer, scheduler):
    ...
    assert load_training_metadata(tmp_path)["num_processes"] == 4
...
def test_load_training_num_processes_absent_returns_none(tmp_path, optimizer, scheduler):
    ...
    assert load_training_metadata(tmp_path).get("num_processes") is None
...
def test_save_training_state_records_batch_size(tmp_path, optimizer, scheduler):
    ...
    assert load_training_metadata(tmp_path)["batch_size"] == 32
...
def test_load_training_batch_size_absent_returns_none(tmp_path, optimizer, scheduler):
    ...
    save_training_state(tmp_path, 10, optimizer, scheduler)
    assert load_training_metadata(tmp_path).get("batch_size") is None


def test_save_training_state_records_gradient_accumulation_steps(tmp_path, optimizer, scheduler):
    save_training_state(tmp_path, 10, optimizer, scheduler, gradient_accumulation_steps=8)
    assert load_training_metadata(tmp_path)["gradient_accumulation_steps"] == 8


def test_load_training_gradient_accumulation_steps_absent_returns_none(tmp_path, optimizer, scheduler):
    # Checkpoints written before accumulation metadata must remain loadable.
    save_training_state(tmp_path, 10, optimizer, scheduler)
    assert load_training_metadata(tmp_path).get("gradient_accumulation_steps") is None
...
def test_save_checkpoint_peft(mock_save_training_state, tmp_path, optimizer):
    ...
    cfg = Mock()
    cfg.peft = None  # Existing adapters can be resumed without a new launch-time PEFT block.
    save_checkpoint(tmp_path, 10, cfg, policy, optimizer)
    policy.save_pretrained.assert_called_once()
    cfg.save_pretrained.assert_called_once()
