...
def test_startup_timeout_must_be_positive():
    with pytest.raises(ValueError, match="startup_timeout_s"):
        RealSenseCameraConfig(serial_number_or_name="042", startup_timeout_s=0)
...
def test_output_resize_is_separate_from_capture_resolution(img_array_factory):
    ...
    color = img_array_factory(height=480, width=640)

    output = camera._postprocess_image(color)

    assert (camera.capture_width, camera.capture_height) == (640, 480)
...
def test_output_letterbox_preserves_aspect_ratio():
    ...
    color = np.full((480, 640, 3), 255, dtype=np.uint8)

    output = camera._postprocess_image(color)

    assert output.shape == (256, 256, 3)
    assert not output[:32].any() and not output[224:].any()
    assert (output[32:224] == 255).all()


def test_output_resize_config_validation(assert_invalid_output_resize_config):
    assert_invalid_output_resize_config(
        lambda **kwargs: RealSenseCameraConfig(serial_number_or_name="042", **kwargs)
    )
