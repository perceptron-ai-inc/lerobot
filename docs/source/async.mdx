...
"""Check out the cameras available in your setup by running `lerobot-find-cameras`"""
...
## Reconnects, the policy cache, and `rename_map`

The policy server keeps the loaded policy in memory across client sessions, keyed on
`(policy_type, pretrained_name_or_path, policy_config_overrides, device)`. A reconnecting
client with the same signature reuses the loaded weights instead of paying the load again —
which for a large VLA is the difference between a two-minute and an instant handshake. Three
properties of that cache are worth knowing:

- **The robot contract is revalidated on every handshake**, cache hit included. If the policy
  implements the contract hook (perceptron_isaac does), a client whose state width, action
  width, camera count, or image size no longer matches the checkpoint fails at readiness with
  a `contract validation failed` error rather than mid-episode with a live arm. The cached
  policy is not evicted by that failure, so a correct client can connect immediately after.
- **`rename_map` is not part of the cache key.** A non-empty client map is patched onto the
  cached preprocessor for that session. The default empty map `{}` **restores the rename map
  packaged inside the checkpoint's own preprocessor** — it does not erase it. This matters for
  checkpoints that ship a map (the map is how their internal camera slot names are reached);
  before this behaviour existed, the second and later sessions against a warm server silently
  lost it and failed to resolve cameras.
- **HF-original mode additionally keys on the handshake features.** When
  `pretrained_name_or_path` is empty, the policy config's input/output features are derived
  from the connecting robot, so a _different_ robot forces a rebuild instead of reusing a
  policy shaped by the first one.
