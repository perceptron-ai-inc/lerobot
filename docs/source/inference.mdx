...
| Flag                                   | Description                                    |
| -------------------------------------- | ---------------------------------------------- |
| `--strategy.ring_buffer_seconds`       | Duration of buffered telemetry (default: 10)   |
| `--strategy.ring_buffer_max_memory_mb` | Memory cap for the ring buffer (default: 1024) |
...
| Flag                                 | Description                                                                             |
| ------------------------------------ | --------------------------------------------------------------------------------------- |
| `--strategy.num_episodes`            | Correction episodes to record (default: unset — falls back to `--dataset.num_episodes`) |
...
### Temporal ensemble (`--inference.type=temporal_ensemble`)

Keeps several recent chunks alive at once and blends the action each of them proposes for the
current instant, weighting by chunk age (`exp(-ensemble_m * age)`). This is the backend the
ISAAC SO100 production path uses.

| Flag                                 | Description                                                                 |
| ------------------------------------ | --------------------------------------------------------------------------- |
| `--inference.ensemble_m`             | Age-decay rate; larger values weight the freshest chunk more (default: 0.5) |
| `--inference.max_step`               | Per-step motion cap applied direction-preserving (default: 15.0)            |
| `--inference.smooth_alpha`           | Output smoothing factor (default: 1.0 = off)                                |
| `--inference.ring_capacity`          | Chunks kept live (default: 6)                                               |
| `--inference.max_consecutive_errors` | Inference failures tolerated before aborting (default: 3)                   |

Very large `ensemble_m` values are safe: once `ensemble_m * age` exceeds ~104 the float32
weights underflow to zero, and the ensemble falls back to the freshest live chunk rather than
dividing by zero. A non-finite blended target is rejected instead of being sent to the robot.
...
| `--fps`                           | Control loop frequency. Adopted from the checkpoint's `target_fps` when it publishes one, else 30. An explicit value that disagrees with `target_fps` is **rejected**, and unless you set `--dataset.fps` explicitly it is rewritten to match. | checkpoint `target_fps`, else 30 |
...
| `--rename_map`                    | Rename observation keys before the policy sees them (JSON dict)                                                                                                                                                                                | `{}`                             |
| `--execute_actions`               | Send predicted actions to the robot (false = prediction only)                                                                                                                                                                                  | true                             |
| `--return_to_initial_position`    | Drive back to the start pose on exit                                                                                                                                                                                                           | true                             |
| `--preflight_inferences`          | Warm-up inferences before the loop starts                                                                                                                                                                                                      | 1                                |
| `--exclusive_device_lock`         | Refuse to start if another process holds the robot                                                                                                                                                                                             | true                             |

**Serving a PEFT (adapter) checkpoint.** `--policy.path` may point at an adapter package whose
`config.json` references sidecar files (normalization stats, render metadata) by a relative
path. Those are resolved against the **adapter** directory first, so an adapter no longer picks
up its base package's sidecars by accident. Perceptron Isaac additionally checks that the
normalization serialized in its processor agrees with the package configuration and rejects an
inconsistent package before inference.
